...
//variable box -s tep one add realtionship between variable and type
...
//new collision logic to handle different actors
...
Level.prototype.actorAt = function(actor) {
  ...
  //checking to see if any other actors intersect
...
Level.prototype.animate = function(step, keys) {
  ...
  while (step > 0) {
    ...
    this.actors.forEach(function(actor) {
      // Allow each actor to act on their surroundings - anything you add has to have an act function defined using this info
...
this.wobble += step x wobbleSpeed;
...
Level.prototype.playerTouched = function(type, actor) {
  if (type == "coin") {
    this.actors = this.actors.filter(function(other) {
      //goes through current set of actors and returns things except the intitial actors
      return other != actor;
    });
  }
};
